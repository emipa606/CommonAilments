...
public class CommonAilments
{
    ...
    {
        var pawnList = PawnsFinder.AllMapsCaravansAndTravellingTransporters_Alive.ToList();
        if (pawnList.Count <= 0)
        {
            return;
        }
    ...
    private static void GetRndAilment(out HediffDef hediffdef, out float severity)
    {
    ...
    private static HediffDef GetBPHed(BodyPartDef partdef)
    {
        HediffDef hediffdef = null;
    ...
    private static bool IsInCryptoCasket(Pawn p)
    {
        return p?.ParentHolder is Building holder &&
               (holder is Building_AncientCryptosleepCasket || holder is Building_CryptosleepCasket);
    }

    private static bool GetBPR(Pawn p, out BodyPartRecord part, out BodyPartDef partdef)
    {
        part = null;
        partdef = null;
    ...
    private static List<string> CAValidRaceBodies()
    {
    ...
    private static List<SickListData> CASickList()
    {
        ...
        out bool planting, out bool dirty)
    {
        ...
        if (phedset != null)
        {
            ...
            if (chkList.Count > 0)
            {
                ...
                {
                    var chkhediff = SickListData.GetSLDHD(chkList[i]);
                    var chkfactor = SickListData.GetSLDF(chkList[i]);
    ...
    private static List<JobDef> CurJobPlantsListing()
    {
    ...
    private static bool CurJobPlants(Job job)
    {
    ...
    private static List<JobDef> CurJobPhysicalListing()
    {
    ...
    private static bool CurJobPhysical(Job job)
    {
        var physical = false;
        var jobdef = job.def;
    ...
    internal struct SickListData
    {
        ...
        internal static HediffDef GetSLDHD(SickListData s)
        {
            return s.SLDdef;
        }

        internal static int GetSLDF(SickListData s)
        {
            return s.SLDfactor;
        }
